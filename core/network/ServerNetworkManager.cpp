 ...
 *
 * Initializes the server manager and prepares internal signal handling and
 * timers needed for running the network service.
 *
...
/**
 * @brief Associate a client identifier with its UDP endpoint for future
 * communication.
 *
 * Stores or updates the mapping from the given client `id` to the provided UDP
 * `endpoint`.
 *
 ...
 *
 * @param buffer Shared pointer to a vector of bytes containing the payload to
...
/**
 * @brief Registers asynchronous OS signal handlers that initiate an orderly
 * shutdown.
 *
 * Sets up an asynchronous wait on the internal signal set so that when a
 * registered signal is received and the server is running, the manager is
 * marked not running, stop() is invoked, and the io_context is stopped.
 */
 ...
 *
 * Sets the manager as not running, cancels signal handlers and any active
 * timers, closes the network socket, and invokes the optional stop callback if
 * provided. If the manager is already stopped, this function returns without
...
/**
 * @brief Begins and continuously receives datagrams from the socket, invoking
 * the provided callback for each received packet.
 *
 * The function keeps posting receive operations while the manager is running
 * and the socket remains open. On a successful receive with data, the callback
 * is invoked with a pointer to the received bytes and the number of bytes.
 * Receive errors other than `asio::error::operation_aborted` are logged as
 * warnings when the manager is running.
 *
 * @param callback Function invoked for each received packet; receives a pointer
 * to the data buffer and the number of bytes received.
 */
...
/**
 * @brief Schedules a recurring event-processing callback to run at the given
 * interval.
 *
 * If an event is already scheduled or the server is not running, this call has
 * no effect. The provided callback is invoked only when the server remains
 * running; after each successful invocation the timer re-schedules itself to
...
/**
 * @brief Schedules recurring checks for unacknowledged packets at a fixed
 * interval.
 *
 * If a check is already scheduled, this call has no effect. When the timer
 * expires and the manager is running, the provided callback is invoked and the
 * check is rescheduled to run again after the same interval.
 *
...
/**
 * @brief Schedule repeated execution of a callback to clear the last processed
 * sequence.
 *
 * Sets up an internal timer to invoke the provided callback every given
 * interval while the server remains running. If a clear-sequence timer is
 * already scheduled or the manager is not running, the call is a no-op. The
 * timer re-schedules itself after each successful invocation.
 *
 * @param interval Duration between consecutive callback invocations.
 * @param callback Function invoked to clear the last processed sequence when
 * the timer expires.
 */
